...
*/
torrentSearchNameSpace={
hideDisplay(){

  document.querySelector("#torrent-quicksearch-msgnode").textContent=""
  document.querySelector("#torrent-quicksearch-overlay").style.setProperty('--icon-size', `${this.iconSmall}%`);
 ...
 showDisplay(){
    ...
    document.querySelector("#torrent-quicksearch-msgnode").style.display="block"
      document.querySelector("#torrent-quicksearch-overlay").style.setProperty('--icon-size', `${this.iconLarge}%`);
...
searchObj:{
  ...
  async search(){
    ...
    if( Array.from(document.querySelectorAll(".torrent-quicksearch-resultitem")).length==0){
        document.querySelector("#torrent-quicksearch-resultlist").textContent="No Matches"
  ...
  async toggleSearch(){
    ...
    else if((content.style.display === "none" ||content.style.display === "")&& this.ready){
      try{
      showDisplay()
      await this.setup()
      await this.doSearch()
...
async searchIndexer(indexerObj,imdb,total,count){
   msg=null
    controller.signal.addEventListener("abort", () => {
   ...
   if (controller.signal.aborted){
    ...
    msg=`Results fetched fom ${this.indexerObj["name"]}:${this.count.length+1}/${this.total} Indexers completed`
...
async searchProwlarrIndexer(indexer){
...
async searchJackettIndexer(indexer){
...
async searchHydra2Indexer(indexer){
...
fetch(url,method="GET",data=null,headers={}){
...
getParser(){
  siteName=this.standardNames[window.location.host] || window.location.host
  data=this.infoParser[siteName]
  if (data===undefined){
...
`
    ...
    e.style.fontSize=`${this.GM_config.get("fontsize",12)}px`
    })
...
addResultsTable(data){
  ...
  data.forEach((e,i)=>{
    ...
    node.innerHTML=`
    <span class="torrent-quicksearch-resultcell">
    ...
    <span class="torrent-quicksearch-resultcell"  >?</span>
    <span class="torrent-quicksearch-resultcell"  >${e['Title']}</span>
    <span class="torrent-quicksearch-resultcell"  >${e['Indexer']}</span>
    <span class="torrent-quicksearch-resultcell" >${e['Grabs']} </span>
  <span class="torrent-quicksearch-resultcell" >${e['Seeders']} </span>
  <span class="torrent-quicksearch-resultcell" >${e['Leechers']} </span>
 <span class="torrent-quicksearch-resultcell" >${e['Cost']} </span>
 <span class="torrent-quicksearch-resultcell"  >${new Date(e['PublishDate']).toLocaleString("en-CA")}</span>
 <span class="torrent-quicksearch-resultcell"  >${(parseInt(e['Size'])/1073741824).toFixed(2)} GB</span>
<span class="torrent-quicksearch-resultcell" >${e['ImdbId']}</span>
  `
     Array.from(node.children).forEach((e,i)=>{
    e.style.gridColumnStart=i+1
    e.style.fontSize=`${this.GM_config.get("fontsize",12)}px`
...
addNumbers(){
  Array.from(document.querySelectorAll(".torrent-quicksearch-resultitem")).forEach((e,i)=>{
    ...
    node.textContent=`${this.i+1}`
  }

  )
},

async  processSonarrNode(node,data){
...
async  processRadarrNode(node,data){
...
box.innerHTML=`
 <div>
  <img id="torrent-quicksearch-toggle" src="${this.searchIcon}"></img>
...
<style>
  ...
  --grid-size: max(calc(50vw/${this.rowSplit}),calc(100%/${this.rowSplit}));
  --icon-size:${this.iconSmall}%;
    --icon-padding:${this.paddingSmall}%;
  }
   #torrent-quicksearch-overlay {
       position: sticky;
       display: flex;
  ...
  padding-bottom:var(--icon-padding);
    margin-bottom:calc(${this.paddingLarge}vh - var(--icon-padding));
  margin-top:calc(${this.paddingLarge}vh - var(--icon-padding));
}
  ...
  position:absolute;
  display:none;
  min-height: ${this.boxMinHeight}vh;
  max-height:${this.boxMaxHeight}vh;
  height: ${this.boxHeight}vh;
  width: ${this.boxWidth}vw;
  max-width: ${this.boxMaxWidth}vw;
  overflow:hidden;
  ...
  #torrent-quicksearch-msgnode{
    ...
    width:calc(var(--grid-size)*${this.rowSplit});
    display:none;
  }
  ...
  display: grid;
    grid-template-columns: repeat(${this.rowSplit},var(--grid-size));
    width:calc((var(--grid-size)*${this.rowSplit})-10);
  }
...
<style/>`
  ...
  box.querySelector("#torrent-quicksearch-toggle").addEventListener("mousedown", this.leftClickProcess)
    box.querySelector("#torrent-quicksearch-toggle").addEventListener("mouseup", this.mouseUpProcess)
      document.addEventListener("mouseup", this.resetMouse)
...
async  tmdbExternalMedia(type,id){
   key=GM_config.get('tmdbapi',"null")
  if(key=="null"){
    return null
  }
  let baseURL=`https://api.themoviedb.org/3/movie/${this.id}/external_ids`
  let params = new URLSearchParams();
  params.append("api_key",key)
  if(type=="tv"){
  baseURL=`https://api.themoviedb.org/3/tv/${this.id}/external_ids`
  }
  searchURL=`${this.baseURL}?${this.params.toString()}`
  req=await fetch(searchURL)
  return JSON.parse(req.responseText)

},

async  imdbTMDBConvertor(imdb){
  key=GM_config.get('tmdbapi',"null")
  imdb=String(imdb)
  if(key=="null"){
    return null
  ...
  let baseURL=`https://api.themoviedb.org/3/find/${this.imdb}`
  let params = new URLSearchParams();
...
async  getIndexersJackett(){
  let params = new URLSearchParams();
  params.append("apikey",GM_config.get('searchapi'));
  //Invalid Category Search
  params.append("Category[]",20130)
  params.append("Query","''")
  baseURL=`${this.GM_config.get('searchurl')}/api/v2.0/indexers/all/results`
  indexerURL=`${this.baseURL}?${this.params.toString()}`
  req=await fetch(indexerURL)
  data=JSON.parse(req.responseText)['Indexers']
  output=[]
  //Same keys as Prowlarr
  for (i in data){
    output.push({"name":data[i]["Name"],"id":data[i]["ID"]})

  }
  return output
},

async  getIndexersProwlarr(){
        let params = new URLSearchParams();
        params.append("apikey",GM_config.get('searchapi'));

        baseURL=`${this.GM_config.get('searchurl')}/api/v1/indexer`
        indexerURL=`${this.baseURL}?${this.params.toString()}`
        req=await fetch(indexerURL)
        return JSON.parse(req.responseText)
...
async  getIndexersHydra(){
        let params = new URLSearchParams();
        params.append("apikey",GM_config.get('searchapi'));
        baseURL=`${this.GM_config.get('searchurl')}/api/stats/indexers/`
        indexerURL=`${this.baseURL}?${this.params.toString()}`
        req=await fetch(indexerURL)
         data=JSON.parse(req.responseText)
      output=[]
    //Same keys as Prowlarr
      for (i in data){
    output.push({"name":data[i]["indexer"],"id":data[i]["indexer"]})
 ...
 indexerCacheHelper(allIndexers){
 ...
 for (let j in indexerNames){
     key=`${this.searchprogram}_${this.indexerNames[j]}`
...
*/
 ...
 getSearchURLProwlarr(indexer){
  let params = new URLSearchParams();
  params.append("apikey",GM_config.get('searchapi'));
  params.append("query",`${this.document.querySelector("#torrent-quicksearch-customsearch").value}`)
 ...
 getSearchURLJackett(indexer){
  let params = new URLSearchParams();
  params.append("apikey",GM_config.get('searchapi'));
  params.append("Query",`${this.document.querySelector("#torrent-quicksearch-customsearch").value}`)
  baseURL= new URL(`/api/v2.0/indexers/${this.indexer}/results`,GM_config.get('searchurl')).toString()
  params.append("cachetime","20")
  return `${this.baseURL}?${this.params.toString()}`

},
 getSearchURLHydraNZB(indexer){
  let params = new URLSearchParams();
  params.append("apikey",GM_config.get('searchapi'));
  params.append("q",`${this.document.querySelector("#torrent-quicksearch-customsearch").value}`)
...
*/
 ...
 leftClickProcess(e){
  ...
  mouseState="down"
  document.addEventListener("mousemove", this.mouseDragProcess)
    document.querySelector("#torrent-quicksearch-overlay").style.setProperty('--icon-padding', `${this.paddingLarge}vh`);
...
//Reset Mouse Events
 resetMouse(){
  mouseState="up"
  document.querySelector("#torrent-quicksearch-overlay").style.setProperty('--icon-padding', `${this.paddingSmall}vh`);
  document.removeEventListener("mousemove", this.mouseDragProcess)
},
 ...
 mouseDragProcess(e){
  ...
  viewport=Math.min(viewport,89)
    document.querySelector("#torrent-quicksearch-overlay").style.top=`${this.viewport}vh`

},
async  sonarrFactory(releaseData){
 sonarrEvent=async function (e){
     e.preventDefault()
     e.stopPropagation()
    res=await fetch(getSonarrURL(),"post", JSON.stringify(releaseData))
    if (res.status!=200){
    ...
    finalMsg=data.reduce(
      ...
      if(curr["rejected"]==true){
        ...
        errorMsg=[`${this.curr["seriesTitle"]} Season ${this.curr["seasonNumber"]} Episodes ${epNums}`,`Status Rejected: ${this.curr["rejections"].join(",")}`]
        return `${this.prev}\n\[${errorMsg.join("\n")}\]`
      }
      if(curr["approved"]==true){
         acceptMsg=`Added ${this.curr["title"]} to client`
         return `${this.prev}\n${this.acceptMsg}`
      }
...
async  radarrFactory(releaseData){
 radarrEvent=async function(e){
     e.preventDefault()
     e.stopPropagation()
    res=await fetch(getRadarrURL(),"post", JSON.stringify(releaseData),{"Content-Type":"application/json"})
    if (res.status!=200){
      GM.notification(res.responseText, program,searchIcon)
      return
    }
    ...
    finalMsg=data.reduce(
      (prev,curr,index)=>{
      if(curr["rejected"]==true){
        errorMsg=[`${this.curr["movieTitles"].join(",")}`,`Status Rejected: ${this.curr["rejections"].join(",")}`]
        return `${this.prev}\n\[${errorMsg.join("\n")}\]`
      }
      if(curr["approved"]==true){
         acceptMsg=`Added ${this.curr["title"]} to client`
...
//Main
 main(){
  this.siteParser=this.getParser()
  this.createMainDOM()
  GM_config.init(
{
...
GM.registerMenuCommand('Torrent Quick Search Settings', ()=> {this.GM_config.open();});

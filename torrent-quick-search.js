...
searchObj={
  async doSearch(){
    ...
    getTableHead()
    document.querySelector("#msgnode").textContent="Fetching Results From Indexers"
    imdb=await getIMDB()
    //reset count
    let count=[]
    ...
    errorMsgs=data.filter((e)=>e["status"]=="rejected").map((e)=>e["reason"])
  ...
  setup(){
    if (typeof this.timeoutID === 'number') {
     ...
     this.timeoutID = setTimeout(this.doSearch(), 1000);
  },
...
async function searchIndexer(indexerObj,imdb,total,count){
    ...
    data=data.filter((e)=>imdbFilter(e,imdb))
    data=data.filter((e)=>currSiteFilter(e["infoUrl"]))

    processResults(data)
    count.push(indexerObj["id"])
    msg=`Fetching Results From Indexers:${count.length}/${total}`
        document.querySelector("#msgnode").textContent=msg
...
function getTableHead(){
    ...
    node=document.querySelector("#resulthead");
    node.innerHTML=`
    <div class="resultcell">Number</div>
    <div class="resultcell"  >Title</div>
    <div class="resultcell"  >Indexer</div>
    <div class="resultcell"  >Grabs</div>
    <div class="resultcell"  >Seeders</div>
    <div class="resultcell"  >Leechers</div>
  <div class="resultcell"  >DLCost</div>
    <div class="resultcell"  >Date</div>
    <div class="resultcell">Size</div>
    <div class="resultcell"  ></div>
     <div class="resultcell" ></div>`
...
function processResults(data){
  ...
  data.forEach((e,i)=>{
    ...
    node.innerHTML=`
    <div class="resultcell"  >?</div>
    <div class="resultcell"  >${e['title']}</div>
    <div class="resultcell"  >${e['indexer']}</div>
    <div class="resultcell" >${e['grabs']} </div>
  <div class="resultcell" >${e['seeders']} </div>
  <div class="resultcell" >${e['leechers']} </div>
 <div class="resultcell" >${e['cost']} </div>

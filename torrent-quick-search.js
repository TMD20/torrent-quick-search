...
function addResultsTable(data){
  ...
  data.forEach((e,i)=>{
    ...
    node.innerHTML=`
    <span class="torrent-quicksearch-resultcell"  ><a href=${e['downloadUrl']}>Download</a><br><a href=${e['infoUrl']}>Details</a></span>
...
boxMaxWidth=150
box.innerHTML=`
<img id="torrent-quicksearch-toggle" src="${searchIcon}">
</img>
...
<div id="torrent-quicksearch-content">
  ...
  <div id="torrent-quicksearch-custombox">
        <div>
        <!--     <label for="torrent-quicksearch-customsearch">Search</label> -->
      <label>Title:</label>
  ...
  #torrent-quicksearch-msgnode{
    ...
    display:none;
     ...
     #torrent-quicksearch-custombox {
      background-color:#FFFFFF;
       width: width:calc(var(--grid-size)*${rowSplit});
...
document.querySelector("#torrent-quicksearch-toggle").addEventListener("mousedown", leftClickProcess)
  document.querySelector("#torrent-quicksearch-customsearchbutton").addEventListener("click",()=>
   {
    let obj=searchObj
    obj.setup()
    customSearch=true
...
function mouseDragProcess(e){
    dragged=true
    boxHeight=parseInt(getComputedStyle(document.querySelector("#torrent-quicksearch-overlay")).height.replaceAll( /[^0-9.]/g, ''));

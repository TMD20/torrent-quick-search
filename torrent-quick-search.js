...
async function doSearch(){
    ...
    imdb=await getIMDB()
    data = await Promise.allSettled(indexers.map((e)=>searchIndexer(e,imdb,indexers.length)));
    addNumbers()
    console.log("Finished Fetching")
    document.querySelector("#msgnode").textContent="Finished"
  }

async function searchIndexer(indexerObj,imdb,total){
    console.log(`Fetching From ${indexerObj["name"]}`)
    document.querySelector("#msgnode").textContent=`Fetching Results From Indexers:${count}/${total}`
...
createMainDOM()
//Use to Keep track of indexer retrival
let count=0
